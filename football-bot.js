	...
	for(user_doc of docs) {
		...
		for(bet of b) {
			if(bet.bet_value.includes(r)) {
				...
				(await client.users.fetch(user_doc.id)).send(`Hai vinto la scommessa con valore ${bet.bet_value} della partita con ID \`${bet.id_partita}\` del <t:${new Date(bet.timestamp).getTime() / 1000}>, ti sono stati aggiunti ${aumento} ${currency_name}.`);
			} else {
				coll.doc(user_doc.id).update({
					lost:  admin.firestore.FieldValue.increment(1)
				});
				(await client.users.fetch(user_doc.id)).send(`Hai perso la scommessa con valore ${bet.bet_value} della partita con ID \`${bet.id_partita}\` del <t:${new Date(bet.timestamp).getTime() / 1000}>, non ti sono stati aggiunti credit. Riprova (o forse no, è meglio non giocare d'azzardo)`);
			}
		}
	}
}
